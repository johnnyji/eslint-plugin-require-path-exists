...
function getWebpackAliases(webpackConfigPath) {
  ...
  if (Array.isArray(webpackConfig)) {
    return webpackConfig.reduce(function (aliases, config) {
      var configAliases = getWebpackAliasesFromSingleConfig(config);
  ...
  throw new Error('Expected Webpack config to be either an object or array, got: ' + (typeof webpackConfig === 'undefined' ? 'undefined' : _typeof(webpackConfig)));
}
...
function testModulePath(value, fileDir) {
  ...
  try {
    _resolve2.default.sync(value, {
      ...
      paths: process.env.NODE_PATH ? [process.env.NODE_PATH] : undefined
    });
  } catch (e) {
    return e.message;
  }
